...
public:
        ...
        : _year(year), _month(month), _day(day)
    {
        ...
        {
            cout << "Parameter invalid,Set the date class to the default date:1900 01 01" << endl;
            _year = 1900;
            _month = 1;
            _day = 1;
        }
    ...
    Date operator+(int days) const
    {
        ...
        // 注意： + 不需要修改左右操作数中的内容，即days和this都不能修改
        // 因此，加完之后结果只能通过临时变量保存起来

        Date temp(*this);
        temp._day += days;

        // 注意，加完之后，一定要检测temp的日期合法性
        // 将日期调成合法的结果
        int daysOfMonth = 0;
    ...
    Date operator-(int days) const
    {
        ...
        // 注意：-不需要修改左右操作数的内容，即days和this都不能被修改
        //  因此加完之后只能通过临时对象保存起来
        Date temp(*this);
        temp._day -= days;
        while (temp._day <= 0)
    ...
    ~Date()
    {
        // 一个疑问？为啥在执行TestDate5()时候，出现执行多条析构函数？什么情况下导致多次创建对象？--离谱。...待验证
        //  cout << "~Date()" << endl;
    }
...
void TestDate1()
{
    ...
    if (d2.IsLeapYear())
    {
        cout << d2.GetYear() << "is leap year!" << endl;
    }
    else
        cout << d2.GetYear() << "is not leap year!" << endl;
